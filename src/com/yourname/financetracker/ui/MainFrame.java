...
public class MainFrame extends JFrame {
	  ...
	  private final FinanceManager manager;
	    ...
	    // --- Show Details with Delete ---
	    private void showDetails() {
	        List<Income> incomes = manager.getIncomes();
	        List<Expense> expenses = manager.getExpenses();
	        List<Debt> debts = manager.getAllDebts();

	        StringBuilder sb = new StringBuilder();
	        sb.append("==== GELİRLER ====\n");
	        for (int i = 0; i < incomes.size(); i++)
	            sb.append(i).append(". Tutar: ").append(incomes.get(i).getAmount())
	              .append(", Tarih: ").append(incomes.get(i).getDate())
	              .append(", Açıklama: ").append(incomes.get(i).getDescription())
	              .append("\n");

	        sb.append("\n==== GİDERLER ====\n");
	        for (int i = 0; i < expenses.size(); i++)
	            sb.append(i).append(". Tutar: ").append(expenses.get(i).getAmount())
	              .append(", Tarih: ").append(expenses.get(i).getDate())
	              .append(", Açıklama: ").append(expenses.get(i).getDescription())
	              .append("\n");

	        sb.append("\n==== BORÇLAR ====\n");
	        for (int i = 0; i < debts.size(); i++)
	            sb.append(i).append(". Tutar: ").append(debts.get(i).getAmount())
	              .append(", Tarih: ").append(debts.get(i).getDate())
	              .append(", Açıklama: ").append(debts.get(i).getDescription())
	              .append(", Son Tarih: ").append(debts.get(i).getDeadline())
	              .append(", Ödendi mi? ").append(debts.get(i).isPaid() ? "Evet" : "Hayır")
	              .append("\n");
	        ...
	        int option = JOptionPane.showOptionDialog(
	                this, scrollPane, "TÜM DETAYLAR",
	                JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE,
	                null, new String[]{"KAPAT", "SİL"}, "KAPAT"
	        );

	        if (option == 1) { // SİL seçildi
	            String indexStr = JOptionPane.showInputDialog(this, "Silmek istediğiniz öğe numarasını girin:");
	            try {
	                int index = Integer.parseInt(indexStr);
	                if (index >= 0 && index < incomes.size()) {
	                    manager.removeIncomeAt(index);
	                } else if (index >= incomes.size() && index < incomes.size() + expenses.size()) {
	                    manager.removeExpenseAt(index - incomes.size());
	                } else if (index >= incomes.size() + expenses.size() &&
	                           index < incomes.size() + expenses.size() + debts.size()) {
	                    manager.removeDebtAt(index - incomes.size() - expenses.size());
	                } else {
	                    showError("Geçersiz numara!");
	                }
	                refresh();
	                FinanceXMLWriter.save(manager);
